...
/* 静态资源批处理 请求服务 */
function requestService(className,server){
    log.info("requestService[className:"+className+", server"+server);
    $("." + className).each(function (index, item) {
        $.ajax({url:"http://"+server+"/"+ $(item).text(),
...
/* 静态资源批处理 请求服务 */
function requestService(className,server,urlAttr){
    log.info( "===发送请求====");
    log.info("requestService[className:"+className+",server"+server+",attr:"+urlAttr);
    log.info( "===发送请求====");
    log.info( "requestCount:"+$("." + className).size());
    $("." + className).each(function (index, item) {
        log.info( "requestNum:"+index);
        $.ajax({url:"http://"+server+"/"+ $(item).attr(urlAttr)+"",
            dataType:"text",
            success: function (reslut) {
                log.log("getServerData:"+reslut);
                if(reslut!==null&&reslut!=="") {
                    var a = document.createElement('a');
                    a.setAttribute("href","/login/personalUserInformation.html");
                    a.innerText = reslut;
                    a.setAttribute("class","a_while");
                    $(item).html(a);
                }
            }
...
function loading(ClassName,url,isLazy,scrollTop) {
    log.info("loading html:"+url);
    var _scrollTop = 0;
    if(isLazy===true) {
        $("." + ClassName).prevAll().each(function (index, item) {
            _scrollTop += parseInt($(item).css("height").replace(new RegExp("px", "g"), ""));
            log.debug("_scrollTop:"+_scrollTop);
        });
        _scrollTop-=100;
        log.debug("_scrollTop:"+_scrollTop);
        if(scrollTop!==null||scrollTop!==""||scrollTop!==undefined)
            _scrollTop = scrollTop;
        function lazyload() {
            if (document.documentElement.scrollTop > _scrollTop) {
                $.ajax({
                    url: url,
                    async: false,
                    success: function (reslut) {
                        $("." + ClassName).html(reslut);
...
function getUrlDate(Url){
    log.info( "send url request,url="+Url);

    $.ajax({url:Url,
        dataType:"text",
        success: function (reslut) {
            log.log("get url,Data="+reslut);
            return reslut;
        }
...
/* 静态资源批处理 请求服务 */
// function requestService(className,server,urlAttr,resultAttr){
//     log.info("requestService[className:"+className+",server"+server+",attr:"+urlAttr);
//     $("." + className).each(function (index, item) {
//         $.ajax({url:"http://"+server+"/"+ $(item).attr(urlAttr),
//             success: function (reslut) {
//                 if(reslut!==null&&reslut!=="")
//                 $(item).attr(resultAttr,reslut)
//             }
//         });
//     });
// }
